...
def normalise_max(alternatives: list[Alternative], criteria: list[Critere]) -> list[Alternative]:
    """Normalise the values of all alternatives following the formula: (v/max(V))"""
...
def normalise_max_min(alternatives: list[Alternative], criteria: list[Critere]) -> list[Alternative]:
    """Normalise the values following the formula: (v-min(V))/(max(V)-min(V))"""
    result = deepcopy(alternatives)
    for crit in criteria:
        max_value = max(alt.values[crit.name] for alt in result)
        ...
        for alt in result:
            if crit.direction == Direction.MAX:
                ...
                alt.values[crit.name] = (1 -(alt.values[crit.name] - min_value) / (max_value - min_value)) * 100 if max_value != min_value else 0
    return result

def normalise_sum(alternatives: list[Alternative], criteria: list[Critere]) -> list[Alternative]:
    """Normalise following the formula: v/sum(V)"""
    result = deepcopy(alternatives)
    for crit in criteria:
        ...
        for alt in result:
            if crit.direction == Direction.MAX:    
                ...
                alt.values[crit.name] = (1 - alt.values[crit.name] / sum_value) * 100 if sum_value != 0 else 0
    return result
...
def save_normalised_data(alternatives: list[Alternative], criteria: list[Critere]):
    ...
    write_list_of_alternatives_to_csv(normalise_max(deepcopy(alternatives), criteria), criteria, file)
    
    # Save "normalised max-min" data to a csv file
    file = output_folder / "normalised_max_min.csv"
    write_list_of_alternatives_to_csv(normalise_max_min(deepcopy(alternatives), criteria), criteria, file)
    
    # Save "normalised sum" data to a csv file
    file = output_folder / "normalised_sum.csv"
    write_list_of_alternatives_to_csv(normalise_sum(deepcopy(alternatives), criteria), criteria, file)
